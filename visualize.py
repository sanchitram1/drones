...
def plot_capacity_sweep(df: pd.DataFrame, title_suffix: str = ""):
    ...
    if len(df) > 2:
        ...
        if len(diffs) > 0:
            ...
            ax.axvline(
                df.iloc[breakdown_idx]["n_drones"],
                color="r",
                ls="--",
                alpha=0.5,
                label=f"Breakdown ≈ {df.iloc[breakdown_idx]['n_drones']} drones",
            )
            ax.legend()
    ...
    fig.suptitle(
        f"Capacity Breakdown Analysis {title_suffix}", fontsize=14, fontweight="bold"
    )
    plt.tight_layout()
    return fig
...
def plot_topology_comparison(results: Dict[str, pd.DataFrame]):
    ...
    for topo_name, df in results.items():
        ...
        axes[0].plot(df["n_drones"], df["n_conflicts"], "o-", color=c, lw=2, label=l)
        axes[1].plot(
            df["n_drones"], df["avg_flight_time_s"] / 60, "s-", color=c, lw=2, label=l
        )
        axes[2].plot(
            df["n_drones"], df["avg_detour_ratio"], "^-", color=c, lw=2, label=l
        )

    axes[0].set_xlabel("Drones")
    axes[0].set_ylabel("Conflicts")
    axes[0].set_title("Conflicts")
    axes[0].legend()
    axes[0].grid(True, alpha=0.3)

    axes[1].set_xlabel("Drones")
    axes[1].set_ylabel("Avg Flight Time (min)")
    axes[1].set_title("Flight Time")
    axes[1].legend()
    axes[1].grid(True, alpha=0.3)

    axes[2].set_xlabel("Drones")
    axes[2].set_ylabel("Detour Ratio")
    axes[2].set_title("Detour Ratio (Manhattan/Euclidean)")
    axes[2].axhline(np.sqrt(2), color="gray", ls=":", label="Max grid penalty √2")
    axes[2].legend()
...
def plot_protocol_comparison(df: pd.DataFrame):
    ...
    axes[0].set_ylabel("Conflicts")
    axes[0].set_title("Total Conflicts")
    ...
    axes[2].set_ylabel("Avg Turns")
    axes[2].set_title("Turns per Route")
...
def plot_3d_trajectories(
    ...
    n_show: int = 10,
    alt_config=None,
    title: str = "3D Drone Trajectories",
):
    ...
    for u, v in topology.G.edges():
        ...
        ax.plot(
            [p1[0], p2[0]], [p1[1], p2[1]], [0, 0], color="lightgray", lw=0.5, alpha=0.5
        )

    # Draw altitude reference planes
    if alt_config and hasattr(topology, "cfg"):
        extent = (
            topology.cfg.grid_extent if hasattr(topology.cfg, "grid_extent") else 2000
        )
        ...
        for alt_val in [
            ...
            ax.plot_surface(Xp, Yp, np.full_like(Xp, alt_val), alpha=0.03, color="blue")
    ...
    for i, mission in enumerate(show_missions):
        ...
        ax.plot(wps[:, 0], wps[:, 1], wps[:, 2], "-", color=color, lw=1.5, alpha=0.8)
...
def plot_grid_heatmap(missions, topology, title="Edge Utilization Heatmap"):
    ...
    for u, v in topology.G.edges():
        if (min(u, v), max(u, v)) not in edge_usage:
            ...
            p2 = topology.get_position(v)
            ax.plot(
                [p1[0], p2[0]], [p1[1], p2[1]], color="lightgray", lw=0.5, alpha=0.3
            )
...
def plot_flight_time_distribution(missions):
    ...
    axes[0].axvline(
        np.mean(times), color="red", ls="--", label=f"Mean: {np.mean(times):.1f} min"
    )
    ...
    detours = [
        m.total_distance / max(m.euclidean_distance, 1)
        for m in missions
        if m.euclidean_distance > 0
    ]
    axes[1].hist(detours, bins=25, color="#2ecc71", alpha=0.8, edgecolor="white")
    axes[1].axvline(
        np.sqrt(2), color="red", ls="--", label=f"Max grid: √2 ≈ {np.sqrt(2):.2f}"
    )
    axes[1].axvline(
        np.mean(detours), color="blue", ls="--", label=f"Mean: {np.mean(detours):.2f}"
    )
...
def generate_all_plots(
    results: Dict, missions: list, topology, alt_config, save_dir: str = "figures"
):
