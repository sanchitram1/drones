...
def load_sf_street_network(
    cfg: SFConfig = None, force_download: bool = False
) -> nx.MultiDiGraph:
    """
    ...
    print(
        f"  Street network: {G_proj.number_of_nodes()} nodes, "
...
def load_sf_buildings(
    ...
    print(
        f"  Buildings: {len(buildings)} total, "
...
def compute_corridor_clearances(
    buildings: gpd.GeoDataFrame, street_graph: nx.MultiDiGraph
) -> dict:
    """
...
def load_sf_census_population(cfg: SFConfig = None) -> gpd.GeoDataFrame:
    ...
    try:
        ...
        url = "https://data.sfgov.org/resource/tmph-tgz9.geojson?$limit=500"
...
def _generate_synthetic_restaurants(cfg: SFConfig) -> pd.DataFrame:
    ...
    df = df[
        (df.lat >= cfg.lat_range[0])
        & (df.lat <= cfg.lat_range[1])
        & (df.lon >= cfg.lon_range[0])
        & (df.lon <= cfg.lon_range[1])
    ].copy()
...
def snap_to_graph(lat: float, lon: float, G: nx.MultiDiGraph) -> int:
