...
class AltitudeConfig:
    ...
    def get_altitude(self, heading_deg: float, n_bands: int = 4) -> float:
        ...
        h = heading_deg % 360
        if n_bands == 4:
            if 315 <= h or h < 45:     return self.north
            elif 45 <= h < 135:        return self.east
            elif 135 <= h < 225:       return self.south
            else:                       return self.west
        else:  # 8-band
            if 337.5 <= h or h < 22.5:   return self.north
            elif 22.5 <= h < 67.5:        return self.northeast
            elif 67.5 <= h < 112.5:       return self.east
            elif 112.5 <= h < 157.5:      return self.southeast
            elif 157.5 <= h < 202.5:      return self.south
            elif 202.5 <= h < 247.5:      return self.southwest
            elif 247.5 <= h < 292.5:      return self.west
            else:                          return self.northwest
    
    def get_direction_label(self, heading_deg: float, n_bands: int = 4) -> str:
        h = heading_deg % 360
        if n_bands == 4:
            if 315 <= h or h < 45:     return "N"
            elif 45 <= h < 135:        return "E"
            elif 135 <= h < 225:       return "S"
            else:                       return "W"
        else:
...
class SimConfig:
    ...
    # Capacity sweep
    drone_counts: List[int] = field(default_factory=lambda: 
...
class ExperimentConfig:
    ...
    turning_protocol: str = "turn_layer"  # "turn_layer", "intersection_cube", "sphereabout"
