...
from simulator import DroneDeliverySimulation
...
def run_abstract_grid_experiments():
    ...
    if all(v is not None for v in comparison.values()):
        ...
        fig.savefig(
            f"{FIGURES_DIR}/topology_comparison.png", dpi=150, bbox_inches="tight"
        )
        plt.close(fig)
    ...
    if protocol_rows:
        ...
        fig.savefig(
            f"{FIGURES_DIR}/protocol_comparison.png", dpi=150, bbox_inches="tight"
        )
        plt.close(fig)
...
def run_sf_experiments():
    ...
    n_orders = 200
    orders = generate_demand(
        n_orders, restaurants, census, pd.DataFrame(), np.random.default_rng(42)
    )
    ...
    from simulator import TurnLayerProtocol
    ...
    if alt_cfg.north < max_bldg + alt_cfg.safety_buffer:
        ...
        print(
            f"  Adjusting altitude bands up by {offset:.0f}m "
    ...
    for nd in drone_counts_sf:
        ...
        print(
            f"  {nd:4d} drones → {result['n_conflicts']:4d} conflicts | "
            f"avg time {result['avg_flight_time_s']:.1f}s"
        )

    sf_df = pd.DataFrame(sf_results)
    sf_df["conflict_rate_per_1000m"] = (
        sf_df["n_conflicts"] / sf_df["avg_distance_m"].clip(lower=1) * 1000
    )
    ...
    if valid:
        export_scenario(
            ...
            ref_lat=sf_cfg.ref_lat,
            ref_lon=sf_cfg.ref_lon,
            n_drones=30,
        )
    ...
    zone_ctrl = ZoneAdmissionController(
        sf_topo, n_zones_per_side=5, max_drones_per_zone=30
    )
...
def generate_summary_report(abstract_results, sf_results):
    ...
    if abstract_results:
        ...
        for label, df in abstract_results.items():
            ...
            rates = (
                df["conflict_rate_per_1000m"].values
                if "conflict_rate_per_1000m" in df.columns
                else df["conflicts_per_drone"].values
            )
            if len(rates) > 2:
                diffs = np.diff(rates)
                if max(diffs) > 0:
...
def main():
    parser = argparse.ArgumentParser(
        description="Decentralized Drone Delivery Simulation"
    )
    parser.add_argument(
        "--phase",
        choices=["abstract", "sf", "all"],
        default="all",
