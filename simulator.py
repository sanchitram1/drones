...
from config import (ExperimentConfig, GridConfig, AltitudeConfig,
    ...
    def _build_grid(self):
        ...
        for r in range(n):
            for c in range(n):
                ...
                # East edge
                if c < n - 1:
                    self.G.add_edge(nid, nid + 1, 
                                   length=bl, edge_type='grid')
                # North edge
                if r < n - 1:
                    self.G.add_edge(nid, nid + n,
                                   length=bl, edge_type='grid')
    ...
    def _add_diagonals(self):
        ...
        for r in range(n - 1):
            for c in range(n - 1):
                ...
                self.G.add_edge(bl_node, tr_node,
                               length=diag_len, edge_type='diagonal')
                # NW diagonal: bottom-right to top-left
                self.G.add_edge(br_node, tl_node,
                               length=diag_len, edge_type='diagonal')
...
class SFTopology:
    ...
    def __init__(self, G_projected: nx.MultiDiGraph, 
                 building_clearances: Optional[dict] = None):
...
class TurnProtocol:
    ...
    def compute_turn_waypoints(self, 
                                intersection_pos: Tuple[float, float],
                                entry_heading: float,
                                exit_heading: float,
                                entry_alt: float,
                                exit_alt: float) -> List[Tuple[float, float, float]]:
        """
    ...
    def compute_turn_time(self, entry_heading: float, exit_heading: float,
                          entry_alt: float, exit_alt: float,
                          drone_cfg: DroneConfig) -> float:
        raise NotImplementedError
...
class TurnLayerProtocol(TurnProtocol):
    ...
    def compute_turn_waypoints(self, intersection_pos, entry_heading,
                                exit_heading, entry_alt, exit_alt):
    ...
    def compute_turn_time(self, entry_heading, exit_heading,
                          entry_alt, exit_alt, drone_cfg):
...
class SpheraboutProtocol(TurnProtocol):
    ...
    def _heading_to_sphere_point(self, heading: float, 
    ...
    def compute_turn_waypoints(self, intersection_pos, entry_heading,
        ...
        entry_pt = self._heading_to_sphere_point(
            (entry_heading + 180) % 360, intersection_pos, entry_alt)
        exit_pt = self._heading_to_sphere_point(
...
class MissionPlanner:
    ...
    def __init__(self, topology, alt_cfg: AltitudeConfig, 
                 drone_cfg: DroneConfig, turn_protocol: TurnProtocol,
                 n_bands: int = 4):
        self.topo = topology
        self.alt = alt_cfg
    ...
    def plan_mission(self, drone_id: int, origin: int, 
                     destination: int, launch_time: float) -> DroneMission:
        ...
        for seg_idx in range(len(path) - 1):
            ...
            if seg_idx > 0 and cur_dir != prev_dir:
                ...
                turn_wps = self.turn.compute_turn_waypoints(
                    intersection_pos, prev_heading, cur_heading,
                    prev_alt, cur_alt
                )
...
class ConflictDetector:
    ...
    def detect_all_conflicts(self, missions: List[DroneMission],
                              topology) -> List[Conflict]:
        """
        ...
        for mission in missions:
            ...
            for seg_idx in range(len(path) - 1):
                ...
                # Edge key (undirected)
                n1, n2 = min(path[seg_idx], path[seg_idx+1]), \
        ...
        for edge_key, records in edge_occupancy.items():
            for i in range(len(records)):
                for j in range(i + 1, len(records)):
                    ...
                    # Temporal overlap?
                    overlap = min(ri['exit'], rj['exit']) - max(ri['enter'], rj['enter'])
                    if overlap > 0:
...
class ZoneAdmissionController:
    ...
    def __init__(self, topology, n_zones_per_side: int = 5,
                 max_drones_per_zone: int = 50):
        self.n_zones = n_zones_per_side
        self.max_per_zone = max_drones_per_zone
        
        # Compute zone boundaries
        all_positions = [topology.get_position(n) for n in 
                        (topology.node_positions.keys() if hasattr(topology, 'node_positions')
...
class DroneDeliverySimulation:
    ...
    def __init__(self, config: ExperimentConfig):
        ...
        self.planner = MissionPlanner(
            self.topology, config.altitude, config.drone,
            self.turn_protocol, self.n_bands
        )
        
        # Conflict detector
        self.detector = ConflictDetector(
            config.sim.conflict_h_threshold,
            config.sim.conflict_v_threshold
        )
    ...
    def run_single(self, n_drones: int, 
        ...
        detour_ratios = [m.total_distance / max(m.euclidean_distance, 1)
    ...
    def _empty_metrics(self, n_drones):
        return {
            'n_drones': n_drones, 'n_conflicts': 0,
            'conflicts_per_drone': 0, 'conflict_rate': 0,
            'avg_flight_time': 0, 'std_flight_time': 0,
            'max_flight_time': 0, 'avg_distance': 0,
            'avg_euclidean': 0, 'avg_detour_ratio': 0,
            'avg_turns': 0, 'total_distance': 0,
            'alt_band_usage': {}, 'missions': [], 'conflicts': [],
        }
    
    def capacity_sweep(self, drone_counts: Optional[List[int]] = None,
        ...
        for n in tqdm(drone_counts, desc="Capacity sweep", disable=not verbose):
            ...
            if verbose:
                print(f"  {n:5d} drones → {metrics['n_conflicts']:5d} conflicts | "
